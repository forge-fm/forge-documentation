...
Scripts in `<svg>` mode use the [D3 visualization library](https://d3-graph-gallery.com/graph/shape.html) by default. However, D3 can be fairly complex to use, and so various [built-in helpers](./d3fx_apr23.md) are also available. We encourage using the helper library, although in more advanced cases you may wish to use D3 directly.
