...
!!! warning "Bounds aren't the same as constraints!"
...
!!! example "Relational bounds expressions"
    ```
    A = `Alice+`Alex+`Adam
    f = `Alice->`Alex + 
        `Adam->`Alex
...
!!! warning "Bounds apply to all states at once!"
...
!!! example "Example `run` mixing both styles"
    ```
